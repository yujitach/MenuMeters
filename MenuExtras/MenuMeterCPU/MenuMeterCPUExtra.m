...
- (NSImage *)image {
	...
	NSImage *currentImage = [NSImage imageWithSize:imageSize flipped:NO
		...
		if (cpuShowTemperature) {
			[self renderSingleTemperatureIntoImage:imageSize atOffset:renderOffset];
		...
		if (cpuDisplayMode & kCPUDisplayHorizontalThermometer) {
			...
			for (uint32_t cpuNum = 0; cpuNum < cpuCount; cpuNum += stride) {
				...
				float yOffset = (imageHeight -
								 ((cpuNum % rowCount) + 1) * thermometerHeight) - 1.0;
...
- (void)renderSingleTemperatureIntoImage:(NSSize)imageSize atOffset:(float)offset {
